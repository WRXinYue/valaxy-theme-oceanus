...
<template>
  <li v-if="category.total" w="full" class="oceanus-sidebar-category inline-flex items-center justify-between" tabindex="0">
    ...
    >
      <div class="i-tabler-chevron-down mr-4 h-5 w-5" :class="collapsable ? 'angle-down' : 'angle-right'" />
    </button>
  </li>
  ...
  <ul v-show="!collapsable" class="oceanus-sidebar-section">
    <li v-for="categoryItem, i in category.children.values()" :key="i" class="oceanus-sidebar-item">
      <template v-if="!isCategoryList(categoryItem)">
        <RouterLink v-if="categoryItem.title" :to="categoryItem.path || ''" class="oceanus-text oceanus-sidebar-item-link" active-class="active">
          <span i-tabler-file-description class="oceanus-list-symbol mr-2 inline-block" />
          <span>{{ getTitle(categoryItem) }}</span>
        </RouterLink>
      </template>
...
.oceanus-sidebar-item {
  ...
  &-link {
    padding-left: var(--oceanus-c-list-link-buffer);
    display: inline-flex;
    align-items: center;
    width: 100%;
