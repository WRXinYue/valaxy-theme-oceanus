  ...
  <article v-if="isMounted" :class="imagePosition && post.cover && 'flex-row-reverse' || post.cover && 'md:text-right'">
    ...
    <div class="m-4">
      ...
      <RouterLink :to="post.path || ''" :aria-label="`Read more about ${post.title}`">
        ...
        <div class="mb-3 text-[var(--oceanus-c-text-secondary)] dark:text-[var(--oceanus-c-text-secondary)]" v-html="post.excerpt" />
      </RouterLink>
...
article {
  ...
  color: var(--oceanus-c-text-secondary);
  margin: 20px 0;
  border-radius: var(--oceanus-c-article-card-rd);
  ...
  .font-secondary {
    font-size: var(--oceanus-c-article-card-font-size-secondary);
    color: var(--oceanus-c-text-secondary);
  }
